...
│   └── <release>.yaml
├── apps.yaml               # Flux Kustomization wrapper for the per-app registry
├── apps/                   # Self-contained per-app folders (see Per-App Folder Pattern)
│   ├── apps-resourceset.yaml  # Registry: one input per app (name, path, dependsOn)
│   └── <app>/              # Everything for one app: HelmRelease, values, namespace, PVCs, secrets, routes, canary
...
### Per-App Folder Pattern (`apps/`)

**Preferred pattern for application workloads** (migration in progress, app by app; paperless is the reference implementation). Everything an app needs lives in one folder: HelmRelease with inline values, HelmRepository, namespace (including network-policy profile labels), PVCs, ExternalSecrets, routes, egress policies, and canary.

`apps/apps-resourceset.yaml` is a thin registry — one input per app with `name`, `path`, and `dependsOn`. The ResourceSet generates a Flux Kustomization per app (substituting cluster-vars, platform-versions, and cluster-versions) with a health check on the app's HelmRelease. The dependency graph stays in the registry because it is cross-app information; everything app-local stays in the folder.

**Adding an app:** create `apps/<app>/` with all resources → add one input to `apps-resourceset.yaml`. Two touchpoints total.
