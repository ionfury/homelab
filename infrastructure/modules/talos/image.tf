...
data "talos_image_factory_extensions_versions" "machine_version" {
  for_each = { for k, v in local.machines : k => v.image }
...
data "talos_image_factory_urls" "machine_image_url_metal" {
  for_each = { for k, v in local.machines : k => v.image if v.image.platform != "" }
...
data "talos_image_factory_urls" "machine_image_url_sbc" {
  for_each = { for k, v in local.machines : k => v.image if v.image.sbc != "" }
